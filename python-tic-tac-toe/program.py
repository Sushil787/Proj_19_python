...
def checkWin():
    if(lists[1]==lists[2] and lists[2] == lists[3]):
        return True
    if(lists[4]==lists[5] and lists[5] == lists[6]):
        return True
    if(lists[7]==lists[8] and lists[8] == lists[9]):
        return True
    if(lists[1]==lists[4] and lists[4] == lists[7]):
        return True
    if(lists[2]==lists[5] and lists[5] == lists[8]):
        return True
    if(lists[3]==lists[6] and lists[6] == lists[9]):
        return True
    if(lists[1]==lists[5] and lists[5] == lists[9]):
        return True
    if(lists[3]==lists[5] and lists[5] == lists[7]):
        return True
    else:
        return False
        
print("Wlcome to Game")
